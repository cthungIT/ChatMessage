...
- (UITableView *)autoCompletionView
{
    if (!_autoCompletionView)
    {
        ...
        _autoCompletionView.scrollsToTop = YES;
...
- (SLKTextInputbar *)textInputbar
{
    if (!_textInputbar)
    {
        ...
        [_textInputbar.layer setCornerRadius:20];
    }
...
{
//    NSLog(@"\n StringTextView: %@",self.textView.text);
//    if (self.textView.text.length > 0) {
//        
//    }else{
//        [self.textView setTextColor:GRAY_COLOR];
//    }
...
- (void)acceptAutoCompletionWithString:(NSString *)string keepPrefix:(BOOL)keepPrefix
{
    ...
    NSMutableAttributedString *att = [[NSMutableAttributedString alloc] initWithAttributedString:textView.attributedText];
//    [att addAttribute:NSForegroundColorAttributeName value:YELLOW_COLOR range:NSMakeRange(range.location - 1, insertionRange.location - range.location)];
//    [textView setAttributedText:att];
    //
    ...
    for (NSString *user in results) {
        [att addAttribute:NSForegroundColorAttributeName value:YELLOW_COLOR range:[textView.text rangeOfString:user]];
    }
    
//    [att addAttribute:NSForegroundColorAttributeName value:GRAY_COLOR range:NSMakeRange(textView.text.length - 1, 1)];
    [textView setAttributedText:att];
    //
